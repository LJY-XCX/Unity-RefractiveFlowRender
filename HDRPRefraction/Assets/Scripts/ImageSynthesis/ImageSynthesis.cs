...
public class ImageSynthesis : MonoBehaviour {
	...
	private void Save(Camera cam, string filename, int width, int height, bool supportsAntialiasing, bool needsRescale)
	{
        ...
        var finalRT =
            RenderTexture.GetTemporary(width, height, depth, format, readWrite, 8);
        var renderRT = (!needsRescale) ? finalRT :
            RenderTexture.GetTemporary(mainCamera.pixelWidth, mainCamera.pixelHeight, depth, format, readWrite, 8);
        var tex = new Texture2D(width, height, TextureFormat.RGB24, false);
