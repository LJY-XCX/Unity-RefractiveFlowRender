...
float Z2EyeDepth(float z) 
{
    if (unity_OrthoParams.w < 0.5)
        return LinearEyeDepth(z,_ProjectionParams); // Unity's LinearEyeDepth only works for perspective cameras.
    else{
						   ...
						   out float3 worldPos, out float3 worldNormal, out float3 worldView)
{
	...
	worldView   = normalize(_WorldSpaceCameraPos.xyz - worldPos.xyz);
}
